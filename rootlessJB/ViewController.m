...
#define in_bundle(obj)                                                         \
  strdup([[[[NSBundle mainBundle] bundlePath]                                  \
      stringByAppendingPathComponent:@obj] UTF8String])

#define failIf(condition, message, ...)                                        \
  if (condition) {                                                             \
    LOG(message);                                                              \
    goto end;                                                                  \
  }
...
#define removeFile(file)                                                       \
  if (fileExists(file)) {                                                      \
    [[NSFileManager defaultManager] removeItemAtPath:@(file) error:&error];    \
    if (error) {                                                               \
      LOG("[-] Error: removing file %s (%s)", file,                            \
          [[error localizedDescription] UTF8String]);                          \
      error = NULL;                                                            \
    }                                                                          \
  }

#define copyFile(copyFrom, copyTo)                                             \
  [[NSFileManager defaultManager] copyItemAtPath:@(copyFrom)                   \
                                          toPath:@(copyTo)                     \
                                           error:&error];                      \
  if (error) {                                                                 \
    LOG("[-] Error copying item %s to path %s (%s)", copyFrom, copyTo,         \
        [[error localizedDescription] UTF8String]);                            \
    error = NULL;                                                              \
  }

#define moveFile(copyFrom, moveTo)                                             \
  [[NSFileManager defaultManager] moveItemAtPath:@(copyFrom)                   \
                                          toPath:@(moveTo)                     \
                                           error:&error];                      \
  if (error) {                                                                 \
    LOG("[-] Error moviing item %s to path %s (%s)", copyFrom, moveTo,         \
        [[error localizedDescription] UTF8String]);                            \
    error = NULL;                                                              \
  }
...
- (void)viewDidLoad {
  ...
  uname(&u);
  if (strstr(u.machine, "iPad5,"))
    psize = 0x1000;
  else
    _host_page_size(mach_host_self(), &psize);
}
...
- (IBAction)jailbreak:(id)sender {
  ...
  if (debug) {
    kern_return_t ret = host_get_special_port(mach_host_self(), HOST_LOCAL_NODE,
                                              4, &taskforpidzero);
    if (ret) {
      ...
      if (psize == 0x1000 && maxVersion("12.1.2")) {
        ...
        // v3ntex is so bad we have to treat it specially for it not to freak
        // out
  ...
  if (maxVersion("11.4.1")) {
    if (remountRootFS())
      LOG("[-] Failed to remount rootfs, no big deal");
  }
  ...
  if (!fileExists("/var/containers/Bundle/.installed_rootlessJB3")) {
    ...
    if (fileExists("/var/containers/Bundle/iosbinpack64")) {
      ...
      if (fileExists("/var/log/jailbreakd-stderr.log"))
    ...
    failIf(!fileExists("/var/containers/Bundle/tweaksupport") ||
               !fileExists("/var/containers/Bundle/iosbinpack64"),
           "[-] Failed to install bootstrap");
    ...
    symlink("/var/containers/Bundle/iosbinpack64/usr/bin/killall",
            "/var/bin/killall");
  ...
  failIf(dumpOffsetsToFile("/var/containers/Bundle/tweaksupport/offsets.data"),
  ...
  FILE *fixed_dropbear =
      fopen((char *)in_bundle("tars/dropbear.v2018.76.tar"), "r");
  ...
  copyFile(in_bundle("bins/jailbreakd"),
           "/var/containers/Bundle/iosbinpack64/bin/jailbreakd");
  ...
  copyFile(in_bundle("bins/pspawn.dylib"),
           "/var/containers/Bundle/iosbinpack64/pspawn.dylib");
  ...
  copyFile(in_bundle("bins/amfid_payload.dylib"),
           "/var/containers/Bundle/iosbinpack64/amfid_payload.dylib");
  ...
  copyFile(in_bundle("bins/TweakInject.dylib"),
           "/var/containers/Bundle/tweaksupport/usr/lib/TweakInject.dylib");
  ...
  if (launch(in_bundle("bins/tester"), NULL, NULL, NULL, NULL, NULL, NULL,
             NULL)) {
    failIf(trustbin("/var/containers/Bundle/iosbinpack64"),
           "[-] Failed to trust binaries!");
    failIf(trustbin("/var/containers/Bundle/tweaksupport"),
           "[-] Failed to trust binaries!");

    // test
    int ret = launch("/var/containers/Bundle/iosbinpack64/test", NULL, NULL,
  ...
  launch("/var/containers/Bundle/iosbinpack64/usr/bin/killall", "-SEGV",
         "dropbear", NULL, NULL, NULL, NULL, NULL);
  failIf(launchAsPlatform(
             "/var/containers/Bundle/iosbinpack64/usr/local/bin/dropbear", "-R",
  ...
  if (!dpd)
    failIf(launchAsPlatform(
               "/var/containers/Bundle/iosbinpack64/usr/local/bin/dropbear",
               "-R", "-E", NULL, NULL, NULL, NULL, NULL),
           "[-] Failed to launch dropbear");

  //------------- launch daeamons -------------//
  //-- you can drop any daemon plist in iosbinpack64/LaunchDaemons and it will
  //be loaded automatically --//

  plists = [[NSFileManager defaultManager]
      contentsOfDirectoryAtPath:
          @"/var/containers/Bundle/iosbinpack64/LaunchDaemons"
                          error:nil];

  for (__strong NSString *file in plists) {
    printf("[*] Adding permissions to plist %s\n", [file UTF8String]);

    file = [@"/var/containers/Bundle/iosbinpack64/LaunchDaemons"
        stringByAppendingPathComponent:file];

    if (strstr([file UTF8String], "jailbreakd")) {
      printf("[*] Found jailbreakd plist, special handling\n");

      NSMutableDictionary *job = [NSPropertyListSerialization
          propertyListWithData:[NSData dataWithContentsOfFile:file]
                       options:NSPropertyListMutableContainers
                        format:nil
                         error:nil];

      job[@"EnvironmentVariables"][@"KernelBase"] =
          [NSString stringWithFormat:@"0x%16llx", KernelBase];
  ...
  launch("/var/containers/Bundle/iosbinpack64/bin/launchctl", "unload",
         "/var/containers/Bundle/iosbinpack64/LaunchDaemons", NULL, NULL, NULL,
         NULL, NULL);
  launch("/var/containers/Bundle/iosbinpack64/bin/launchctl", "load",
         "/var/containers/Bundle/iosbinpack64/LaunchDaemons", NULL, NULL, NULL,
         NULL, NULL);

  sleep(1);

  failIf(!fileExists("/var/log/testbin.log"),
         "[-] Failed to load launch daemons");
  failIf(!fileExists("/var/log/jailbreakd-stdout.log"),
         "[-] Failed to load jailbreakd");
  ...
  if (self.enableTweaks.isOn) {
    ...
    if (!fileExists(xpcproxy)) {
      ...
      const char *args[] = {
          "insert_dylib", "--all-yes", "--inplace", "--overwrite",
          dylib,          xpcproxy,    NULL};
      int argn = 6;
      ...
      failIf(
          system_(
              "/var/containers/Bundle/iosbinpack64/usr/local/bin/jtool --sign "
              "--inplace --ent /var/containers/Bundle/iosbinpack64/default.ent "
              "/var/libexec/xpcproxy"),
          "[-] Failed to resign xpcproxy!");
    }
    ...
    if ([self.installiSuperSU isOn]) {
      ...
      removeFile(
          "/var/containers/Bundle/tweaksupport/Applications/iSuperSU.app");
      copyFile(in_bundle("apps/iSuperSU.app"),
               "/var/containers/Bundle/tweaksupport/Applications/iSuperSU.app");

      failIf(
          system_(
              "/var/containers/Bundle/tweaksupport/usr/local/bin/jtool --sign "
              "--inplace --ent "
              "/var/containers/Bundle/tweaksupport/Applications/iSuperSU.app/"
              "ent.xml "
              "/var/containers/Bundle/tweaksupport/Applications/iSuperSU.app/"
              "iSuperSU && /var/containers/Bundle/tweaksupport/usr/bin/inject "
              "/var/containers/Bundle/tweaksupport/Applications/iSuperSU.app/"
              "iSuperSU"),
          "[-] Failed to sign iSuperSU");
      ...
      failIf(launch("/var/containers/Bundle/tweaksupport/usr/bin/uicache", NULL,
                    NULL, NULL, NULL, NULL, NULL, NULL),
             "[-] Failed to install iSuperSU");
    }

    // kill any daemon/executable being hooked by tweaks (except for the
    // obvious, assertiond, backboardd and SpringBoard)

    NSArray *tweaks = [[NSFileManager defaultManager]
        contentsOfDirectoryAtPath:@"/var/ulb/TweakInject"
                            error:NULL];
      ...
      if ([afile hasSuffix:@"plist"]) {
        ...
        NSDictionary *plist = [NSDictionary
            dictionaryWithContentsOfFile:
                [NSString stringWithFormat:@"/var/ulb/TweakInject/%@", afile]];
        NSString *dylibPath =
            [afile stringByReplacingOccurrencesOfString:@".plist"
                                             withString:@".dylib"];
        fixMmap((char *)[[NSString stringWithFormat:@"/var/ulb/TweakInject/%@",
                                                    dylibPath] UTF8String]);
        NSArray *executables =
            [[plist objectForKey:@"Filter"] objectForKey:@"Executables"];

        for (NSString *processName in executables) {
          if (![processName isEqual:@"SpringBoard"] &&
              ![processName isEqual:@"backboardd"] &&
              ![processName isEqual:@"assertiond"] &&
              ![processName isEqual:@"launchd"]) { // really?
        ...
        NSArray *bundles =
            [[plist objectForKey:@"Filter"] objectForKey:@"Bundles"];
        for (NSString *bundleID in bundles) {
          if (![bundleID isEqual:@"com.apple.springboard"] &&
              ![bundleID isEqual:@"com.apple.backboardd"] &&
              ![bundleID isEqual:@"com.apple.assertiond"] &&
              ![bundleID isEqual:@"com.apple.launchd"]) {
            NSString *processName =
                [bundleID stringByReplacingOccurrencesOfString:@"com.apple."
                                                    withString:@""];
            int procpid = pid_of_procName((char *)[processName UTF8String]);
            if (procpid) {
              kill(procpid, SIGKILL);
    ...
    // find which applications are jailbreak applications and inject their
    // executable
    NSArray *applications = [[NSFileManager defaultManager]
        contentsOfDirectoryAtPath:@"/var/containers/Bundle/Application/"
                            error:NULL];

    for (NSString *string in applications) {
      NSString *fullPath = [@"/var/containers/Bundle/Application/"
          stringByAppendingString:string];
      NSArray *innerContents =
      ...
      for (NSString *innerFile in innerContents) {
        if ([innerFile hasSuffix:@"app"]) {
          ...
          NSString *fullAppBundlePath = [fullPath
              stringByAppendingString:[NSString
                                          stringWithFormat:@"/%@", innerFile]];
          NSString *_CodeSignature = [fullPath
              stringByAppendingString:
                  [NSString stringWithFormat:@"/%@/_CodeSignature", innerFile]];

          NSDictionary *infoPlist =
              [NSDictionary dictionaryWithContentsOfFile:
                                [NSString stringWithFormat:@"%@/Info.plist",
                                                           fullAppBundlePath]];
          NSString *executable = [infoPlist objectForKey:@"CFBundleExecutable"];
          NSString *BuildMachineOSBuild =
              [infoPlist objectForKey:@"BuildMachineOSBuild"];
          ...
          // check for keys added by native/appstore apps and exclude (theos and
          // friends don't add BuildMachineOSBuild and DT_ on apps :-D )
          // Xcode-added apps set CFBundleExecutable=Executable, exclude them
          // too

          executable = [NSString
              stringWithFormat:@"%@/%@", fullAppBundlePath, executable];
          ...
          if (([[NSFileManager defaultManager]
               ...
               ![[NSFileManager defaultManager]
                   fileExistsAtPath:_CodeSignature] ||
               (executable && ![executable isEqual:@"Executable"] &&
                !BuildMachineOSBuild & !hasDTCompilerRelatedKeys)) &&
              fileExists([executable UTF8String])) {

            LOG("Injecting executable %s", [executable UTF8String]);
            system_((char *)[[NSString
                stringWithFormat:
                    @"/var/containers/Bundle/iosbinpack64/usr/bin/inject %s",
                    [executable UTF8String]] UTF8String]);
          }
        }
      }
    }
    ...
    LOG("TASK_DYLD_INFO patch");
    uint64_t host_priv_kaddr = FindPortAddress(mach_host_self());
    uint64_t realhost_kaddr = KernelRead_64bits(host_priv_kaddr + 0x68);
    uint64_t port_kaddr =
        KernelRead_64bits(realhost_kaddr + 0x10 + 4 * sizeof(void *));
    uint64_t fake_task_kaddr = KernelRead_64bits(port_kaddr + 0x68);
    LOG("host_priv_kaddr:0x%llx", host_priv_kaddr);
    LOG("realhost_kaddr:0x%llx", realhost_kaddr);
    LOG("port_kaddr:0x%llx", port_kaddr);
    LOG("fake_task_kaddr:0x%llx", fake_task_kaddr);
    KernelWrite_64bits(fake_task_kaddr + 0x398, KernelBase);
    KernelWrite_64bits(fake_task_kaddr + 0x3a0, KASLR_Slide);
    LOG("saved kernelbase & slide to TASK_DYLD_INFO");
    term_jelbrek();

    // bye bye
    kill(bb, 9);
    // launch("/var/containers/Bundle/iosbinpack64/bin/bash", "-c",
    // "/var/containers/Bundle/iosbinpack64/usr/bin/nohup
    // /var/containers/Bundle/iosbinpack64/bin/bash -c
    // \"/var/containers/Bundle/iosbinpack64/bin/launchctl unload
    // /System/Library/LaunchDaemons/com.apple.backboardd.plist &&
    // /var/containers/Bundle/iosbinpack64/usr/bin/ldrestart;
    // /var/containers/Bundle/iosbinpack64/bin/launchctl load
    // /System/Library/LaunchDaemons/com.apple.backboardd.plist\" 2>&1 >/dev/null
    // &", NULL, NULL, NULL, NULL, NULL);
    exit(0);
  }
  ...
  /*
   ...
   failIf(!entitlePidOnAMFI(installd, "get-task-allow", true), "[-] Failed to
   entitle installd"); failIf(!entitlePidOnAMFI(installd,
   "com.apple.private.skip-library-validation", true), "[-] Failed to entitle
   installd");

   inject_dylib(installd,
   "/var/containers/Bundle/tweaksupport/usr/lib/TweakInject/AppSyncUnified.dylib");

   if ([self.installiSuperSU isOn]) {
   LOG("[*] Installing iSuperSU");
   copyFile(in_bundle("apps/iSuperSU.app"),
   "/var/containers/Bundle/tweaksupport/Applications/iSuperSU.app");
   launch("/var/containers/Bundle/tweaksupport/usr/bin/uicache", NULL, NULL,
   NULL, NULL, NULL, NULL, NULL);
   } */
...
- (IBAction)uninstall:(id)sender {
  ...
  failIf(!fileExists("/var/containers/Bundle/.installed_rootlessJB3"),
         "[-] rootlessJB was never installed before! (this version of it)");
